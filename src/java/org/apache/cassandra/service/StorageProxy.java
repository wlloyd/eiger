...
public class StorageProxy implements StorageProxyMBean
{
     ...
     */
    public static void sendMessages(String localDataCenter, Map<String, Multimap<Message, InetAddress>> dcMessages, IWriteResponseHandler handler)
    throws IOException
    {
                                             ...
                                             final ConsistencyLevel consistency_level)
    {
        return new DroppableRunnable(StorageService.Verb.MUTATION)
        {
            ...
            public void runMayThrow() throws IOException
            {
                ...
                logger.debug("counterWriteTask: " + cm + " to " + targets + " sROW: " + cm.shouldReplicateOnWrite());
    ...
    private static List<Row> fetchRows(List<ReadCommand> initialCommands, ConsistencyLevel consistency_level) throws IOException, UnavailableException, TimeoutException
    {
        ...
        do
        {
            ...
            for (int i = 0; i < commands.size(); i++)
            {
                ...
                {
                    assert false : "Proxying reads is not currently allowed in COPS2 " + dataPoint + " != " + handler.endpoints.get(0) + " or " + OPTIMIZE_LOCAL_REQUESTS;
            ...
            if (repairResponseHandlers != null)
            {
                for (int i = 0; i < repairCommands.size(); i++)
                {
                    ...
                    //
                    // WL NOTE: Waiting on the repair writes to be acked is what ensures LOCAL_QUORUM provides
                    // linearizability inside each datacenter.  It makes it classic linearizable quorums ala
                    // Attiya, Bar-Noy and Dolev.  (Well the read part of that protocol at least).
    ...
    throws IOException, UnavailableException, TimeoutException
    {
        ...
        try
        {
            ...
            for (AbstractBounds<RowPosition> range : ranges)
            {
                ...
                if ((consistency_level == ConsistencyLevel.ONE || overallReplicationFactor == 1 || dcReplicationFactor == 1)
                        && !liveEndpoints.isEmpty() && liveEndpoints.contains(FBUtilities.getBroadcastAddress()))
                {
                    if (logger.isDebugEnabled())
