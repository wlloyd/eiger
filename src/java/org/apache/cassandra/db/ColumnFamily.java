...
import static org.apache.cassandra.db.DBConstants.boolSize;
import static org.apache.cassandra.db.DBConstants.intSize;
import static org.apache.cassandra.db.DBConstants.longSize;
...
public class ColumnFamily extends AbstractColumnContainer
{
    ...
    public void addCounter(QueryPath path, long value, long timestamp)
    {
        addCounter(path, value, timestamp, timestamp, null);
    }
    ...
    {
        assert path.columnName != null : path;
        CounterUpdateColumn column = new CounterUpdateColumn(path.columnName, value, timestamp, transactionCoordinatorKey);
        column.setEarliestValidTime(earliestValidTime);
        addColumn(path.superColumnName, column);
    }
    ...
    {
        addTombstone(path, localDeletionTime, timestamp, timestamp, null);
    }

    public void addTombstone(QueryPath path, ByteBuffer localDeletionTime, long timestamp, long earliestValidTime, ByteBuffer transactionCoordinatorKey)
    {
        assert path.columnName != null : path;
        DeletedColumn column = new DeletedColumn(path.columnName, localDeletionTime, timestamp, transactionCoordinatorKey);
        column.setEarliestValidTime(earliestValidTime);
        addColumn(path.superColumnName, column);
    }
    ...
    public void addTombstone(QueryPath path, int localDeletionTime, long timestamp, long earliestValidTime, ByteBuffer transactionCoordinatorKey)
    {
        assert path.columnName != null : path;
        DeletedColumn column = new DeletedColumn(path.columnName, localDeletionTime, timestamp, transactionCoordinatorKey);
        column.setEarliestValidTime(earliestValidTime);
    ...
    {
        addTombstone(name, localDeletionTime, timestamp, timestamp, null);
    }

    public void addTombstone(ByteBuffer name, int localDeletionTime, long timestamp, long earliestValidTime, ByteBuffer transactionCoordinatorKey)
    {
        //WL Note: Tombstone timestamp added here (or above)
        DeletedColumn column = new DeletedColumn(name, localDeletionTime, timestamp, transactionCoordinatorKey);
        column.setEarliestValidTime(earliestValidTime);
