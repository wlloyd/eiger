 ...
 *         nodeid_1          nodeid_2
 *
 *  and COPS adds the logical timestamp for each update at the end of each entry
 *
...
public class CounterContext implements IContext
{
    ...
    private static final int TIMESTAMP_LENGTH = DBConstants.longSize;
    private static final int COORDINATOR_KEY_LENGTH_LENGTH = DBConstants.intSize;
    private static final int STEP_LENGTH = NodeId.LENGTH + CLOCK_LENGTH + COUNT_LENGTH + TIMESTAMP_LENGTH + COORDINATOR_KEY_LENGTH_LENGTH; //+ actual coordinatorKeys too
     ...
     */
    public ByteBuffer create(long value, long timestamp, ByteBuffer coordinatorKey, Allocator allocator)
    {
        ...
        writeElementAtOffset(context, context.position() + HEADER_SIZE_LENGTH + HEADER_ELT_LENGTH, NodeId.getLocalId(), 1L, value, timestamp, coordinatorKey);
        return context;
    }
    ...
    {
        return create(id, clock, value, LamportClock.getVersion(), isDelta);
    }


    // Provided for use by unit tests
    public ByteBuffer create(NodeId id, long clock, long value, long timestamp, boolean isDelta)
    {
        ...
        writeElementAtOffset(context, context.position() + HEADER_SIZE_LENGTH + (isDelta ? HEADER_ELT_LENGTH : 0), id, clock, value, timestamp, ByteBufferUtil.EMPTY_BYTE_BUFFER);
        return context;
    }

    // write a tuple (node id, clock, count) at an absolute (bytebuffer-wise) offset
    private static void writeElementAtOffset(ByteBuffer context, int offset, NodeId id, long clock, long count, long timestamp, ByteBuffer coordinatorKey)
    {
        ...
        context.putLong(timestamp);
        if (coordinatorKey == null || coordinatorKey.remaining() == 0) {
            context.putInt(0);
        } else {
            context.putInt(coordinatorKey.remaining());
            context.put(coordinatorKey);
        }
    }
            ...
            if (cmp == 0)
            {
                if (leftState.isDelta() || rightState.isDelta())
                {
                    ...
                    if (leftState.isDelta() && rightState.isDelta())
                    {
                        ...
                        //WL TODO: Is this correct?
                        if (leftState.getTimestamp() > rightState.getTimestamp()) {
                            mergedState.writeElement(leftState.getNodeId(), clock, count, leftState.getTimestamp(), leftState.getCoordinatorKey(), true);
                        } else {
                            mergedState.writeElement(rightState.getNodeId(), clock, count, rightState.getTimestamp(), rightState.getCoordinatorKey(), true);
                        }
                    }
                    else
                    {
    ...
    public HashMap<Long, ByteBuffer> timestampToCoordinatorKey(ByteBuffer context)
    {
        HashMap<Long, ByteBuffer> timestampToCoordinatorKey = new HashMap<Long, ByteBuffer>();

        for (int offset = context.position() + headerLength(context); offset < context.limit(); offset += STEP_LENGTH)
        {
            //TODO: Could shorten these by using ShortNodeIds instead of NodeIds
            long timestamp = context.getLong(offset + NodeId.LENGTH + CLOCK_LENGTH + COUNT_LENGTH);
            int coordinatorKeyLength = context.getInt(offset + NodeId.LENGTH + CLOCK_LENGTH + COUNT_LENGTH + TIMESTAMP_LENGTH);
            offset += coordinatorKeyLength;

            if (coordinatorKeyLength == 0) {
                timestampToCoordinatorKey.put(timestamp, ByteBufferUtil.EMPTY_BYTE_BUFFER);
            } else {
                ByteBuffer coordinatorKey = ByteBuffer.allocate(coordinatorKeyLength);
                int coordinatorKeyOffset = offset + NodeId.LENGTH + CLOCK_LENGTH + COUNT_LENGTH + TIMESTAMP_LENGTH + COORDINATOR_KEY_LENGTH_LENGTH;
                ByteBufferUtil.arrayCopy(context, coordinatorKeyOffset, coordinatorKey, 0, coordinatorKeyLength);
                timestampToCoordinatorKey.put(timestamp, coordinatorKey);
            }
        }

        return timestampToCoordinatorKey;
    }
    ...
    public ByteBuffer computeOldShardMerger(ByteBuffer context, List<NodeId.NodeIdRecord> oldIds, long mergeBefore)
    {
        ...
        while (state.hasRemaining())
        {
            ...
            {
                merger.writeElement(localId, 1L, mergeTotal, LamportClock.getVersion(), ByteBufferUtil.EMPTY_BYTE_BUFFER, true);
                localWritten = true;
            }
            else if (i < toMerge.size() && nodeId.compareTo(toMerge.get(i)) == 0)
            {
                ...
                removedTotal += count;
                merger.writeElement(nodeId, -now - state.getClock(), -count, state.getTimestamp(), ByteBufferUtil.EMPTY_BYTE_BUFFER, true);
                ++i;
            }
            state.moveToNext();
        }
    ...
    public static class ContextState
    {
        ...
        {
            return context.getLong(context.position() + bodyOffset + NodeId.LENGTH + CLOCK_LENGTH + COUNT_LENGTH);
        }

        public ByteBuffer getCoordinatorKey()
        {
            int coordinatorKeyLength = context.getInt(context.position() + bodyOffset + NodeId.LENGTH + CLOCK_LENGTH + COUNT_LENGTH + TIMESTAMP_LENGTH);
            if (coordinatorKeyLength == 0) {
                return null;
            } else {
                ByteBuffer coordinatorKey = ByteBuffer.allocate(coordinatorKeyLength);
                int coordinatorKeyOffset = NodeId.LENGTH + CLOCK_LENGTH + COUNT_LENGTH + TIMESTAMP_LENGTH + COORDINATOR_KEY_LENGTH_LENGTH;
                ByteBufferUtil.arrayCopy(context, coordinatorKeyOffset, coordinatorKey, 0, coordinatorKeyLength);
        ...
        public void writeElement(NodeId id, long clock, long count, long timestamp, ByteBuffer coordinatorKey, boolean isDelta)
        {
            writeElementAtOffset(context, context.position() + bodyOffset, id, clock, count, timestamp, coordinatorKey);
            if (isDelta)
            {
        ...
        {
            writeElement(id, clock, count, LamportClock.getVersion(), ByteBufferUtil.EMPTY_BYTE_BUFFER, isDelta);
        }

        //provided for use by unit tests
        public void writeElement(NodeId id, long clock, long count)
        {
            writeElement(id, clock, count, LamportClock.getVersion(), ByteBufferUtil.EMPTY_BYTE_BUFFER, false);
        }

        public void writeElement(NodeId id, long clock, long count, long timestamp)
        {
            writeElement(id, clock, count, timestamp, ByteBufferUtil.EMPTY_BYTE_BUFFER, false);
        }
