...
public class TestRingCache
{
    ...
    public TestRingCache(String keyspace, ClientContext clientContext) throws IOException
    {
    ...
    public static void main(String[] args) throws Throwable
    {
        ...
        TestRingCache tester = new TestRingCache(keyspace, new ClientContext());
        ...
        for (int nRows = minRow; nRows < maxRow; nRows++)
        {
            ...
            tester.thriftClient.set_keyspace(keyspace, LamportClock.sendTimestamp());
            tester.thriftClient.insert(row, parent, new Column(ByteBufferUtil.bytes("col1")).setValue(ByteBufferUtil.bytes("val1")).setTimestamp(1), ConsistencyLevel.ONE, new HashSet<Dep>(), LamportClock.COPS_UNSUPPORTED);
            Column column = tester.thriftClient.get(row, col, ConsistencyLevel.ONE, LamportClock.COPS_UNSUPPORTED).value.column;
