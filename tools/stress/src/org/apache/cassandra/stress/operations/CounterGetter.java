...
public class CounterGetter extends Operation
{
    ...
    public CounterGetter(Session client, int index, ClientContext clientContext)
    {
        super(client, index);
        ...
        for (int j = 0; j < session.getSuperColumns(); j++)
        {
            ...
            for (int t = 0; t < session.getRetryTimes(); t++)
            {
                ...
                try
                {
                    ...
                    counters = client.get_slice(key, parent, predicate, session.getConsistencyLevel(), LamportClock.COPS_UNSUPPORTED).value;
    ...
    private void runCounterGetter(SlicePredicate predicate, Cassandra.Client client) throws IOException
    {
        ...
        for (int t = 0; t < session.getRetryTimes(); t++)
        {
            ...
            {
                List<ColumnOrSuperColumn> counters;
                counters = client.get_slice(keyBuffer, parent, predicate, session.getConsistencyLevel(), LamportClock.COPS_UNSUPPORTED).value;
                success = (counters.size() != 0);
            }
            catch (Exception e)
            {
