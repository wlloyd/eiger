...
public class IndexedRangeSlicer extends Operation
{
    ...
    public IndexedRangeSlicer(Session client, int index, ClientContext clientContext)
    {
        super(client, index);
    ...
    public void run(Cassandra.Client client) throws IOException
    {
        ...
        while (received < expectedPerValue)
        {
            ...
            for (int t = 0; t < session.getRetryTimes(); t++)
            {
                ...
                try
                {
                    results = client.get_indexed_slices(parent, clause, predicate, session.getConsistencyLevel(), LamportClock.COPS_UNSUPPORTED).value;
