...
public class CounterAdder extends Operation
{
    ...
    public CounterAdder(Session client, int index, ClientContext clientContext)
    {
        super(client, index);
    ...
    public void run(Cassandra.Client client) throws IOException
    {
        ...
        for (int t = 0; t < session.getRetryTimes(); t++)
        {
            ...
            try
            {
                client.batch_mutate(record, session.getConsistencyLevel(), new HashSet<Dep>(), LamportClock.COPS_UNSUPPORTED);
                success = true;
            }
            catch (Exception e)
            {
