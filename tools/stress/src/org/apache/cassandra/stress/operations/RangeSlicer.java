...
public class RangeSlicer extends Operation
{
    ...
    public RangeSlicer(Session client, int index, ClientContext clientContext)
    {
        super(client, index);
        ...
        if (session.getColumnFamilyType() == ColumnFamilyType.Super)
        {
            ...
            for (int i = 0; i < session.getSuperColumns(); i++)
            {
                ...
                for (int t = 0; t < session.getRetryTimes(); t++)
                {
                    try
                    {
                        slices = client.get_range_slices(parent, predicate, range, session.getConsistencyLevel(), LamportClock.COPS_UNSUPPORTED).value;
