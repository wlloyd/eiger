...
public class Reader extends Operation
{
    ...
    public Reader(Session client, int index, ClientContext clientContext)
    {
        super(client, index);
        ...
        for (int j = 0; j < session.getSuperColumns(); j++)
        {
            ...
            for (int t = 0; t < session.getRetryTimes(); t++)
            {
                ...
                try
                {
                    ...
                    columns = client.get_slice(key, parent, predicate, session.getConsistencyLevel(), LamportClock.COPS_UNSUPPORTED).value;
    ...
    private void runColumnReader(SlicePredicate predicate, Cassandra.Client client) throws IOException
    {
        ...
        for (int t = 0; t < session.getRetryTimes(); t++)
        {
            ...
            {
                List<ColumnOrSuperColumn> columns;
                columns = client.get_slice(keyBuffer, parent, predicate, session.getConsistencyLevel(), LamportClock.COPS_UNSUPPORTED).value;
                success = (columns.size() != 0);
            }
            catch (Exception e)
            {
