...
#
# Note: Generate with Thrift 0.7 for consistency.
#
# Note: Generate from this with 'ant gen-thrift-java'
#
...
const string VERSION = "19.25.0+cops.2.0.3"

#
# COPS2 new data structures
#

/** Lamport timestamps are immensely useful.  Notably they "provide a
 * partial ordering of all distributed events in a way that respects
 * potential causality."
 *
 * To be used properly they need to be attached to every message sent
...
struct CounterColumn {
    ...
    3: optional i64 deleted_time=0,
    4: optional map<LamportTimestamp, binary> timestampToCoordinatorKey,
    5: optional i64 earliest_valid_time,
    6: optional i64 latest_valid_time,
}
...
#
# Composite read return that all include a lamport timestamp.
#
struct GetResult {
    1: ColumnOrSuperColumn value,
    2: LamportTimestamp lts,
}
struct GetSliceResult {
    1: list<ColumnOrSuperColumn> value,
    2: LamportTimestamp lts,
}
struct GetCountResult {
    1: i32 value,
    2: set<Dep> deps,
    3: LamportTimestamp lts,
}
...
struct CountWithMetadata {
    ...
    3: i64 latest_valid_time,
    4: set<Dep> deps,
}
struct MultigetCountResult {
    1: map<binary, CountWithMetadata> value,
    3: LamportTimestamp lts,
}
struct GetRangeSlicesResult {
    1: list<KeySlice> value,
    2: LamportTimestamp lts,
}
struct GetIndexedSlicesResult {
    1: list<KeySlice> value,
    2: LamportTimestamp lts,
}
...
service Cassandra {
  ...
  # set keyspace
  LamportTimestamp set_keyspace(1: required string keyspace, 99: LamportTimestamp lts) throws (1:InvalidRequestException ire),
   ...
   */
  GetResult get(1:required binary key,
   ...
   */
  GetSliceResult get_slice(1:required binary key,
  ...
  */
  GetCountResult get_count(1:required binary key,
                2:required ColumnParent column_parent, 
                3:required SlicePredicate predicate,
                4:required ConsistencyLevel consistency_level=ConsistencyLevel.ONE,
                99: LamportTimestamp lts)
  ...
  /**
    ...
    If chosen_time is set, node will return the version of a key that was visible at that time.
    Otherwise they return the currently visible value.
  */
  ...
  */
  MultigetCountResult multiget_count(1:required list<binary> keys,
                2:required ColumnParent column_parent,
                3:required SlicePredicate predicate,
                4:required ConsistencyLevel consistency_level=ConsistencyLevel.ONE,
                99: LamportTimestamp lts)
  ...
  MultigetCountResult multiget_count_by_time(1:required list<binary> keys,
                2:required ColumnParent column_parent,
                3:required SlicePredicate predicate,
                4:required ConsistencyLevel consistency_level=ConsistencyLevel.ONE,
                5:required i64 chosen_time,
  ...
  */
  GetRangeSlicesResult get_range_slices(1:required ColumnParent column_parent,
  ...
  GetRangeSlicesResult get_range_slices_by_time(1:required ColumnParent column_parent,
                                  2:required SlicePredicate predicate,
                                  3:required KeyRange range,
                                  6:required list<binary> knownKeys,
                                  4:required ConsistencyLevel consistency_level=ConsistencyLevel.ONE,
                                  5:required i64 chosen_time,
                                  99: LamportTimestamp lts)
                 throws (1:InvalidRequestException ire, 2:UnavailableException ue, 3:TimedOutException te),
  ...
  /**
    ...
    */
  GetIndexedSlicesResult get_indexed_slices(1:required ColumnParent column_parent,
   ...
   */
  WriteResult insert(1:required binary key,
              2:required ColumnParent column_parent,
              3:required Column column,
              4:required ConsistencyLevel consistency_level=ConsistencyLevel.ONE,
              5:required set<Dep> deps,
              99: LamportTimestamp lts)
       throws (1:InvalidRequestException ire, 2:UnavailableException ue, 3:TimedOutException te),
   ...
   */
   WriteResult add(1:required binary key,
           2:required ColumnParent column_parent,
   ...
   */
  WriteResult remove(1:required binary key,
              2:required ColumnPath column_path,
   ...
   */
  WriteResult remove_counter(1:required binary key,
                      2:required ColumnPath path,
                      3:required ConsistencyLevel consistency_level=ConsistencyLevel.ONE,
                      4:required set<Dep> deps,
                      99: LamportTimestamp lts)
      throws (1:InvalidRequestException ire, 2:UnavailableException ue, 3:TimedOutException te),
  ...
  **/
  BatchMutateResult batch_mutate(1:required map<binary, map<string, list<Mutation>>> mutation_map,
                    2:required ConsistencyLevel consistency_level=ConsistencyLevel.ONE,
                    3:required set<Dep> deps,
                    99: LamportTimestamp lts)
       throws (1:InvalidRequestException ire, 2:UnavailableException ue, 3:TimedOutException te),
     
  //Note: I got NullPointerExceptions from thrift's processing code when I tried to use 'oneway void', an i16 is returned here only to avoid those exceptions   
  i16 transactional_batch_mutate_cohort(1:required map<binary, map<string, list<Mutation>>> mutation_map,
                    3:required binary coordinator_key,
  ...
  BatchMutateResult transactional_batch_mutate_coordinator(1:required map<binary, map<string, list<Mutation>>> mutation_map,
                    2:required ConsistencyLevel consistency_level=ConsistencyLevel.ONE,
                    3:required set<Dep> deps,
                    4:required binary coordinator_key,
                    5:required set<binary> all_keys,
                    6:i64 transaction_id,
                    99: LamportTimestamp lts)
       throws (1:InvalidRequestException ire, 2:UnavailableException ue, 3:TimedOutException te),
  ...
  /**
   ...
   some hosts are down.
  */
  LamportTimestamp truncate(1:required string cfname, 2:required set<Dep> deps, 99: LamportTimestamp lts)
       throws (1: InvalidRequestException ire, 2: UnavailableException ue, 3: TimedOutException te),
  ...
  //WL TODO: Add lamport timestamps to the Meta-APIs and nodeprobe too?
